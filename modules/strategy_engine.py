...
def density_score(df: pd.DataFrame, center: float, band: float = 0.03, n: int = 50) -> float:
    ...
    if temp.empty:
        return np.nan
    return float(temp["rate"].between(center - band, center + band).mean())
...
def market_heat_score(density: float, avg_bidders: float) -> float:
    ...
    return round(float(density * bidder_factor * 100), 2)


def difficulty_level(heat_score: float, density: float, avg_bidders: float) -> str:
    if pd.isna(heat_score): return "판단불가"
    if heat_score >= 70: return "매우어려움"
    if heat_score >= 45: return "어려움"
    if heat_score >= 25: return "보통"
    return "낮음"
...
def strategy_comment(rec: dict) -> str:
    ...
    if difficulty == "매우어려움":
        ...
        comments.append("단순 평균 추종보다 기관 최근 흐름과 구간 분산 전략이 필요합니다.")
    elif difficulty == "어려움":
        comments.append("시장 참여자가 평균 구간에 비교적 많이 몰리는 구간입니다.")
        comments.append("중립형 중심 접근이 가능하나 공격형 투찰은 신중해야 합니다.")
    elif difficulty == "보통":
        comments.append("경쟁 난이도는 보통 수준입니다. 최근 흐름과 변동성을 함께 반영하십시오.")
    elif difficulty == "낮음":
        comments.append("밀집도와 업체수 기준 과열 부담은 상대적으로 낮습니다.")
    else:
        comments.append("난이도 판단을 위한 데이터가 부족합니다.")
    if not pd.isna(density): comments.append(f"최근 밀집도는 {density:.1%} 수준입니다.")
    if not pd.isna(avg_bidders): comments.append(f"최근 평균 업체수는 약 {avg_bidders:.1f}개입니다.")
...
def recommend_rate(df: pd.DataFrame, agency: str | None = None, category: str | None = None) -> dict:
    ...
    adj = min(base_adj, 0.03) if difficulty == "매우어려움" else min(base_adj, 0.05) if difficulty == "어려움" else base_adj
    rec = {
        "status":"산출완료", "base":base, "stable":base+adj, "neutral":base, "aggressive":base-adj,
        "volatility":std, "density":density, "avg_bidders":bidders, "heat_score":heat,
        "difficulty":difficulty, "sample_count":int(temp["rate"].notna().sum()), "adjust_width":adj,
    }
